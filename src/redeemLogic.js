...
export function normalizeSubscriptionResult(payload) {
  ...
  if (explicitError) {
    return {
      ...
      subscriptionReason: errorReason || "订阅接口返回失败"
    };
  }
...
export function normalizeSubscriptionError(message) {
  return {
    ...createEmptySubscriptionState(),
    subscriptionStatus: "error",
    subscriptionReason: String(message || "订阅检查失败").trim()
  };
}
